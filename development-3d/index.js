/*
 * Copyright 2025 ayyoub co. All rights reserved.
 *
...
class VoiceCommandSystem {
  ...
  processCommand(transcript, confidence = 0) {
    ...
    this.updateVoiceFeedback(`Processing: ${transcript}`, confidence);

    // If confidence is very low, show suggestions
    if (confidence < 0.3) {
      this.suggestSimilarCommands(transcript);
      return;
    }

    // Rest of the processCommand implementation...
    // ... existing code ...
  }
...
// Initialize 3D test
const threeDTest = new ThreeDTest();
threeDTest.createTestCube();
threeDTest.animate();

function initializeEntrance(scene) {
    // Add sales display near entrance
    createHotspot(scene, {
        yaw: -0.15,
        pitch: 0,
        title: "🎁 SPECIAL SALE! 🎁",
        text: "Welcome to our Special Sale!\n\n" +
              "🔥 Amazing Discounts Across the Store:\n" +
              "• Up to 50% off on selected items\n" +
              "• Buy 2 Get 1 Free on dairy products\n" +
              "• Special bundle deals in every section",
        type: 'info',
        cssClass: 'sale-hotspot'
    });

    // Add 3D sale sign model
    modelSystem.loadModel(
        'sale-sign',
        'models/sale_sign.glb',  // You'll need to provide this 3D model
        { x: 0, y: 2, z: -3 },   // Position above the floor
        0.5                      // Scale
    ).then(model => {
        if (model) {
            // Add floating animation
            const floatAnimation = new THREE.AnimationClip('float', 1, [
            ...
            const action = mixer.clipAction(floatAnimation);
    ...
    giftPositions.forEach((pos, index) => {
        modelSystem.loadModel(
            `gift-box-${index}`,
            'models/gift_box.glb',  // You'll need to provide this 3D model
            pos,
            0.3
        );
    });
